...
class BroadcastAudience(StrEnum):
    """Кому адресована массовая рассылка.

    В базе не хранится: рассылка живёт в памяти процесса. Место в общем
    модуле перечислений выбрано потому, что выборка аудитории —
    это запрос к таблицам, и репозиторий должен понимать её без импорта
    прикладного слоя.
    """

    #: Все, кто не забанен и не заблокировал бота.
    ALL = "all"
    #: Только с оплаченной действующей подпиской.
    ACTIVE = "active"
    #: Пробовали бота и ушли: триал был, действующей подписки нет.
