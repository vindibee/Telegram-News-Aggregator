...
<p align="center">
  Telegram-бот, который собирает записи публичных каналов, склеивает повторы,
  хранит их в PostgreSQL и публикует в ваши каналы — с подписками, пробным
  периодом, оплатой и панелью администратора.
</p>

<p align="center">
  <a href="https://t.me/dekelia_bot"><b>▶ Попробовать бота — @dekelia_bot</b></a>
</p>
...
Бот читает публичные каналы через веб-превью (`t.me/s/<channel>`), сохраняет
каждую запись в PostgreSQL и показывает её пользователю вместе с текстом и
медиа. Поверх этого — законченный продукт: платные тарифы, пробный период,
рефералы, трекинг ссылок и отложенная публикация.

| Возможность | Описание |
|---|---|
| **Лента новостей** | Читает любой публичный канал через веб-превью — без юзербота, MTProto и номера телефона |
| **Дедупликация** | Одна новость из пяти каналов схлопывается в одну запись: SHA-256 → срезы simhash → Jaccard/Левенштейн |
| **Полнотекстовый поиск** | `/search` по архиву: `tsvector`, стемминг, ранжирование и подсветка |
| **Автопостинг** | Публикует выбранные новости в ваши каналы через очередь с повторами |
| **Фильтр слов** | Триггеры и стоп-слова для каждого пользователя |
| **Подписки** | Платные тарифы с пробным периодом и защитой от мультиаккаунтов |
| **Оплата** | Telegram Stars и криптовалюта (CryptoBot), обе идемпотентны от начала до конца |
| **Трекинг ссылок** | Подменяет внешние ссылки, считает переходы и уникальных посетителей, отчёт по `/stats` |
| **Привлечение** | Реферальная программа и промокоды |
| **Админ-панель** | Бизнес-метрики и рассылка с соблюдением лимитов Bot API |
| **Локализация** | Русский, английский и украинский, включая множественные формы |
| **Анти-флуд** | Token bucket в Redis, скользящее окно, single-flight и блокировки критических действий |

## Стек технологий

| Слой | Выбор | Почему |
|---|---|---|
| Язык | **Python 3.11** | `asyncio` повсюду: и бот, и воркер, и приёмник вебхуков упираются в I/O |
| Фреймворк бота | **aiogram 3** | Асинхронный клиент Bot API с роутерами, FSM, middleware и типизированной `callback_data` |
| База данных | **PostgreSQL 15** | Корректность продукта живёт в схеме: уникальные и частичные индексы, CHECK, `FOR UPDATE SKIP LOCKED`, advisory-локи, генерируемые колонки `tsvector` |
| ORM | **SQLAlchemy 2.0** (async) | Типизированные декларативные модели поверх `asyncpg`, весь SQL заперт в репозиторном слое |
| Миграции | **Alembic** | Приложение не меняет схему — это делают только миграции |
| Кэш и блокировки | **Redis 7** | Хранилище FSM, вёдра лимитов, окно дедупликации, буфер кликов и распределённые блокировки |
| Планировщик | **APScheduler** | Периодические задачи в отдельном процессе-воркере |
| HTTP | **aiohttp** | Чтение каналов, загрузка медиа, API CryptoBot и сервер вебхуков/редиректов |
| Парсинг | **BeautifulSoup 4** | HTML веб-превью канала |
| Тесты | **pytest** + **pytest-asyncio** | Настоящий PostgreSQL и `fakeredis[lua]`, время заморожено через `freezegun` |
| Поставка | **Docker Compose** | Бот, воркер, PostgreSQL и Redis — четыре сервиса |

### Ключевые библиотеки

```text
aiogram==3.31.0          фреймворк Telegram Bot API (роутеры, FSM, middleware)
SQLAlchemy==2.0.29       асинхронная ORM, типизированные модели
asyncpg==0.29.0          драйвер PostgreSQL
alembic==1.13.1          миграции схемы
redis==5.0.3             асинхронный клиент Redis (лимиты, FSM, локи, буферы)
APScheduler==3.10.4      планирование фоновых задач
aiohttp==3.9.3           асинхронный HTTP-клиент и сервер вебхуков
beautifulsoup4==4.12.3   разбор HTML превью каналов
python-dotenv==1.0.1     конфигурация из .env

pytest==9.1.1            прогон тестов
pytest-asyncio==1.4.0    асинхронные тесты
pytest-cov==7.0.0        покрытие
freezegun==1.5.1         заморозка времени (с real_asyncio=True)
fakeredis[lua]==2.26.2   Redis под тестами, вместе с Lua-скриптами
```
...
Всё остальное — локальный запуск без Docker, полный справочник переменных
окружения и проектные решения по каждой подсистеме — описано ниже.
...
<p align="center">
  ...
  <a href="https://github.com/vindibee/Telegram-News-Aggregator">Исходники на GitHub</a>
</p>
